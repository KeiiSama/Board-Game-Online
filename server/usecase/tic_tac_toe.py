...
def click(x, y):
    global board, colors, win, lose, move_history
...
def initialize(size):
    global win, lose, board, move_history, user_queue, matches
    user_queue = []
    matches = []
...
def get_match_index(user):
    ...
    for i in range(len(matches)):
        if user in matches[i]["player"]:
            index = i
    return index


def get_match(index):
    state = is_win(matches[index]["board"])
    if state == "Black won":
        matches[index]["winner"] = matches[index]["is_black"]
    elif state == "White won":
        matches[index]["winner"] = matches[index]["is_white"]
    return matches[index]


def play(index, user, x, y):
    if (user != matches[index]["turn"]) or (matches[index]["board"][y][x] != " "):
        return False

    sign = " "
    if user == matches[index]["is_black"]:
        sign = "b"
    if user == matches[index]["is_white"]:
        sign = "w"
    matches[index]["board"][y][x] = sign

    for player in matches[index]["player"]:
        if player != user:
            matches[index]["turn"] = player
